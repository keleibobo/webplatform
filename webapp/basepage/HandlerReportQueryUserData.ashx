<%@ WebHandler Language="C#" Class="HandlerExtendRights2" %>
...
public class HandlerExtendRights2 : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        object[] args = new object[] { "BusinessName=" + value[0], "ComponentName=" + value[1], "EditColumnName=" + value[2], "appname=" + value[3], "param=ReportQuery" };
        ...
        for (int i = 1; i <= userData.datadesclist.Count; i++)
        {
            if (first)
            {
                json += "{\"id\":" + ( i + 10000 ) + ",\"view\":\"false\",\"desc\":\"" + userData.datadesclist[i - 1] + "\",\"ExtendStr\":\"" + userData.datalist[i - 1] + "\"}";
                first = false;
            }
            else
            {
