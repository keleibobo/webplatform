<%@ WebHandler Language="C#" Class="GetAddHtml" %>
...
public class GetAddHtml : IHttpHandler, IRequiresSessionState
{
            ...
            foreach (TableInfo row in TbInfoList)  //根据tableinfo业务字段生成输入表格
            {
                ...
                switch (Convert.ToString(row.ColumnType))
                {
                    ...
                    case "defaultsvg":
                        ...
                        if (DEP.ContainsKey(ExentdName.ToLower()))
                        {
                            st = HtmlControl.GetControl_DefaultSvgSelect(businessname, extendMark + columnName, columnName);
                        }
                        sb.Append(st);
                        tc.Text = st;
                        break;
                    ...
                    case "datagrid":
                        ...
                        if (row.datasource != null && row.datasource != "" && !IScontract && !HasSrc)
                        {
                            st = HtmlControl.GetDataGridHtml(row.ColumnDes,columnName, paramstr);
                        }
                        sb.Append(st); tc.Text = st;
                        break;
                    case "float":
