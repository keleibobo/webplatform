<%@ WebHandler Language="C#" Class="AFHandlerChart" %>
...
public class AFHandlerChart : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string webcomm_StatistCycle = request.Params["webcomm_StatistCycle"];
        string sPara = request.QueryString.ToString();
        string rs = getChartData(sid, sPara, charttype);
        context.Response.Write(rs);
    }

    public string getChartData(string sid, String sPara, string charttype)
    {
        string rs = "[]";
        ...
        switch ((HtmlComponetType)Enum.Parse(typeof(HtmlComponetType), chartype, true))
        {
            case HtmlComponetType.curvedlinechart:
                ...
                {
                    rs = AFHighcharts.getOptions(dtData.dt, options, new AppCode.AFSplineSeries());
                }
                else if (charttype == "column")
                {
                    rs = AFHighcharts.getColumnOptions(dtData.dt);
                }
                else if (charttype == "pie")
                {
                    rs = AFHighcharts.getPieOptions(dtData.dt);
                }
                break;
            default: break;
        }
