<%@ WebHandler Language="C#" Class="GetUpdateHtml" %>
...
public class GetUpdateHtml : IHttpHandler, IRequiresSessionState
{
    ...
    {
        string extendMark = "extendpro";
        HttpRequest request = context.Request;
        HttpResponse response = context.Response;
        if (request.Form["type"] != "")
        {
            ComponentName = request.Form["type"];
        }

        string NameStr = request.Form["name"];
        string ValueStr = request.Form["value"];
        ...
        if (TbInfoList.Count > 0)
        {
            ...
            foreach (TableInfo row in TbInfoList)
            {
                ...
                switch (Convert.ToString(row.ColumnType))
                {
                    ...
                    case "varchar":
                        ...
                        if (DEP.ContainsKey(ExentdName.ToLower()))
                        {
                            columnName = extendMark + columnName;
                        }
                                        ...
                                        if (ti.ColumnName == te.src)
                                        {
                                            foreach (TableInfo dest in TbInfoList)
                                            {
                                                if (dest.ColumnName == te.dest)
                                                {
                                                    ...
                                                    if (columnName.Contains(extendMark))
                                                    {
                                                        destcolumnName = extendMark + dest.ColumnName;
                                                    }
                                                    if (VD.ContainsKey(row.ColumnName.ToLower()))
                                                    {
                    ...
                    case "defaultsvg":
                        ...
                        if (DEP.ContainsKey(ExentdName.ToLower()))
                        {
                            string value = VD[row.ColumnName.ToLower()];
                            st = HtmlControl.GetControl_DefaultSvgSelect(businessname, extendMark + columnName, columnName,value);
                        }
                        sb.Append(st);
                        break;
                    case "checktable":
                    ...
                    case "datagrid":
                        ...
                        if (row.datasource != null && row.datasource != "" && !IScontract && !HasSrc)
                        {
                            ...
                            if (VD.ContainsKey(row.ColumnName.ToLower()))
                            {
                                json = VD[row.ColumnName.ToLower()];
                            }
                            st = HtmlControl.GetDataGridHtml(row.ColumnDes, columnName, paramstr, json);
                        }
                        sb.Append(st);
                        break;
                    case "float":
            ...
            sb.Append("<td colspan=2><div style='margin:0 auto;width:200px'>"
             ...
             + String.Format("<div style='cursor:pointer;width:80px;float:right' id='btncancel' onclick='$(\"#ModalWindow\").window(\"close\");' onmouseout='changeoutimg(this)' onmouseover='changeonimg(this)'>{0}</div>", HtmlCondition.GetControl_Button("取消"))
